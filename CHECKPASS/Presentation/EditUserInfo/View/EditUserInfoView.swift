        ...
        if editUserInfoViewModel.departments == nil {
            ...
            ZStack {
                ...
                .onChange(of: updateName, perform: { newValue in
                    editUserInfoViewModel.compare(newValue,
                                                  userInfoViewModel.detailedUserInfo?.userName)
                })
                .onChange(of: updateDepartment, perform: { newValue in
                    editUserInfoViewModel.compare(newValue, 
                                                  userInfoViewModel.detailedUserInfo?.userDepartment)
                })
                .onChange(of: updateHireDate, perform: { newValue in
                    if let staffInfo = userInfoViewModel.detailedUserInfo as? DetailedStaffInfo {
                        editUserInfoViewModel.compare(newValue,
                                                      staffInfo.hireDate)
                    }
                })
                .onChange(of: updateStudentGrade, perform: { newValue in
                    if let studentInfo = userInfoViewModel.detailedUserInfo as? DetailedStudentInfo {
                        editUserInfoViewModel.compare(newValue, studentInfo.studentGrade)
                    }
                })
                .onChange(of: updateDayOrNight, perform: { newValue in
                    if let studentInfo = userInfoViewModel.detailedUserInfo as? DetailedStudentInfo {
                        editUserInfoViewModel.compare(newValue, studentInfo.dayOrNight)
                    }
                })
                .onChange(of: updateStudentSemester, perform: { newValue in
                    if let studentInfo = userInfoViewModel.detailedUserInfo as? DetailedStudentInfo {
                        editUserInfoViewModel.compare(newValue, studentInfo.studentSemester)
                    }
                })
                .onDisappear {
